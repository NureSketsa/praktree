...
int main(int argc, char const *argv[])
{
    ...
    SinglyList myList, secList;

    // PENTING! Jangan lupa diinisialisasi
    slist_init(&myList);

    int loop, input_num = 0, temp = 0, kth, kth_sec, size = 0, size_sec = 1, temp_kth;
    ...
    while (input_num != -1)
    {
        slist_pushBack(&myList, input_num);
        size++;
        cin >> input_num;
        // temp = input_num;
    }
    ...
    // cout << "size " << (size) << endl;
    // cout << "at " << (kth) << " " << (kth_sec) << endl;
    // cout << "num " << slist_getAt(&myList, kth - 1) << " " << slist_getAt(&myList, kth_sec - 1) << endl;

    int num_one = slist_getAt(&myList, kth - 1), num_two = slist_getAt(&myList, kth_sec - 1);

    slist_removeAt(&myList, kth - 1);
    slist_removeAt(&myList, kth_sec - 2);

    slist_insertAt(&myList, kth - 1, num_two);
    slist_insertAt(&myList, kth_sec - 1, num_one);
